...
6. **サイト内お知らせを更新**：`v○.○/news.md`（自動ドラフト済）を清書し、`src/data/releases.ts` の先頭に
   このバージョンのエントリを足す（これがサイト反映の唯一の作業。先頭＝最新が新機能ページに出る）
7. X（タグ `#ラストサバイニブ #ジョジョLS #イニブ` 固定）→ Discordコミュニティ の順で投稿し、Issueのチェックを埋める

> サイト内「お知らせ」（オンボーディングの新機能ページ）は `src/data/releases.ts` が唯一の出どころ。
> 先頭が最新＝主役、全件が「これまでのアップデート」一覧に並ぶ。`updates/v○.○/news.md` は下書き＋アーカイブ。
