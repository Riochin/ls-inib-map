...
| 3 | 15〜21 | クリップ detail: ピンをタップ → 店舗パネル → 「詳細を見る」→ 店舗詳細（1 画面に収まるのでスクロールしない） | 「出どころつきの情報で / 安心して行ける」 |
...
| クリップ | 操作 | 長さ |
|---|---|---|
| map | 日本全体で 1 秒静止 → 東京圏のまとまったピン（画面内で件数最大）を中心にホイールで 4 段ズームイン → 1.5 秒静止 | 約 6 秒 |
| filter | 絞り込みボタン → ラスサバ・4台〜・録画台あり → 「N件を表示」→ 1.5 秒静止 | 約 6 秒 |
| detail | namco松戸のピンをタップ → 店舗パネル → 「詳細を見る」→ 詳細モーダルで 3 秒静止 | 約 5.5 秒 |

補足（実装時に分かったこと）:
- 渋谷の店は同じ建物の別店舗と常にまとまったピンになり単体でタップできないため、周囲に他店が無く情報が揃っている namco松戸 に変更
- 詳細モーダルは内容が 1 画面に収まりスクロールの余地が無いため、スクロール操作は無し
- スクリーンキャストは `--force-device-scale-factor=3` で Chrome を起動しないと CSS ピクセル寸法（375×812）でしか届かない
- 「閉じる」ボタンは Google マップ内の不可視ボタンにも付いているため、詳細モーダル内のものを座標クリックする
